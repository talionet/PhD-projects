...
class LearnObject: 
    ...
    def run(self,Model='svc',kernel='linear',is_cross_validation=True, cross_validationMethod='LOO', DecompositionMethod='PCA',decompositionLevel='FeatureType',n_components=30, FeatureSelection='TopExplainedVarianceComponents', n_features=10, isPerm=0,isBetweenSubjects=True,isConcatTwoLabels=False,isSaveCsv=None, isSavePickle=None, isSaveFig=None,isSelectSubFeatures=False,SubFeatures='ExpressionLevel'):       
        ...
        isMultiLabels=True
        globalVars.isMultiLabels=isMultiLabels
        ...
        NegativeSymptoms=['N1','N2','N3','N4','N5','N6','N7']
        PositiveSymptoms=['P1','P2','P5']
        GeneralSymptoms=['G7','G11']
        LabelingList=GeneralSymptoms#trainLabels_all.columns #['N1']
        self.ResultsDF=DF()
        ...
        for label_ind, Labeling in LabelingIndex:
            ...
            trainScores=DF()
            test_index=X.index #subjects+segments
            test_index2=testLabels.index #only subjects
            testScores=Panel(items=range(len(cv)),major_axis=['Ytest','Ypredicted'],minor_axis=Labeling)
            testScores2=Panel(items=range(len(cv)),major_axis=['Ytest','Ypredicted'],minor_axis=Labeling)
            #concat([DF(index=testLabels.index),DF(index=['std_train_err'])]) 
            ...
            bestNfeaturesDF=DF(index=range(len(cv)),columns=range(n_features))
            ...
            for train, test in cv:
                ...
                if isBetweenSubjects:
                    ...
                    bestNfeaturesDF.loc[cv_ind]=bestNfeatures
                    ...
                    #train 1 
                    TrainModel=learningUtils.MultipleRegression()
                    TrainModel.fit(Xtrain.sort_index(),Ytrain.sort_index())
                    Ypredicted_AllLabels=TrainModel.predict(Xtest).T
                    #scoresSummary=TrainModel.calcError(Ypredicted_AllLabels,Ytest)
                    #CONTINUE HERE - ALSO TO SECOND LEARNING - ALL LABELS AND ONE LABEL AT A TIME!
                    #calc pseudo inverse
                  ...
                  #  ModelWeights1[cv_ind]=TrainModel.coef_.flatten()
                    ...
                    if isBoolLabel:
                       ...
                       TrainModel2=TrainModel#svm.SVC(kernel='linear', probability=True,class_weight={0:1,1:1})
                    else:
                       try:
                            PiecePrediction_train=DF(TrainModel.decision_function(Xtrain),index=Xtrain.index,columns=['prediction'])
                       except AttributeError:
                            PiecePrediction_train=TrainModel.predict(Xtrain).T

                       TrainModel2=learningUtils.MultipleRegression()
                    ...
                    XtfrainAllFeatures=X.query('Piece_ind == '+ str(list(train_pieces)))
                ...
                if isBoolLabel:
                    ...
                    if isMultiLabels:
                        PiecePrediction_test=TrainModel.predict(Xtest).T
                    else:
                ...
                if cv_ind==0:
                    ...
                    if not(isMultiLabels): #fix
                        YpredictedOverAllLabels[cv_ind][test_subjects]=DF(testScores2[cv_ind].loc['Ypredicted'])#check this
                    elif isMultiLabels:
                        YpredictedOverAllLabels[test_subjects]=DF(testScores2[cv_ind].loc['Ypredicted'])#check this
                    #plt.figure(1)
                    #if len(LabelingList)>1:
                        #plt.subplot(round(len(LabelingList)/2),2,label_ind+1)
                #test2
                    ...
                    if isMultiLabels:
                        ...
                        YpredictedOverAllLabels[cv_ind]=testScores2
           ...
           # else:
            ...
            if isMultiLabels:
                saveName=raw_input('enter save name (.csvge): ')
                scoresSummary.to_csv('C:\\Users\\taliat01\\Desktop\\TALIA\\Code-Python\\Results\\'+saveName)
            ...
            WeightedFeatures1_index0=analysisUtils.getFeaturesWeights(0,bestNfeaturesDF,ModelWeights1) #FeatureAnalysisIndex=0 for featureType, 1= au's (if not decomposed) or component rank (if decomposed)
            WeightedFeatures1_index1=analysisUtils.getFeaturesWeights(1,bestNfeaturesDF,ModelWeights1)
        ...
        self.BestNFeaturesAll=bestNfeaturesDF 
...
def main(isLoadData=1,isCutData=0, PieceLength=500, isLoadFeatures=1,isGetFeaturesNaNs=0, isLoadLabels=1, LabelFileName={}, FeatureMethod='Quantization' ,LabelBy='PatientsVsControls'):
    # -- TODO:
    # -- # 
    ...
    ModelList=['ridge']
    DecompositionList=['FeatureType_PCA']#'noDecomposition','PCA']#,,'noDecomposition'] #['PCA','noDecomposition','KernelPCA','SparsePCA','ICA']
    DecompositionLevel=[]#,'fs-signal']
    FeatureSelectionListWithDecomposition=['TopNComponents']#'f_regression']#'TopExplainedVarianceComponents']#,'FirstComponentsAndFregression',]#[,'FirstComponentsAndExplainedVar'] 
...
LabelByList=['PANSS']#'boolMentalStatus','PatientsVsControls'
for label in LabelByList:
    print('************************************ LabelBy = ' +label+ '************************************')
